...
window.onload = function () {
  ...
  // Iterate over each item to set initial styles and event listeners
  items.forEach((item, index) => {
    // Set initial rotation and transform origin using GSAP
    gsap.set(item, {
    ...
    item.addEventListener("mouseover", function () {
      // Animate the item's position for a hover effect
      gsap.to(item, {
        x: -28,                // Move along X-axis
        y: 10,                 // Move along Y-axis
        z: 10,                 // Move along Z-axis
        ease: "power2.out",    // Easing function
        duration: 0.5,         // Animation duration
      });
    });

    // Add a mouseout event listener to reset the item when the mouse leaves
    item.addEventListener("mouseout", function () {
      // Animate the item back to its original position
      gsap.to(item, {
        x: 0,                  // Reset X-axis position
        y: 0,                  // Reset Y-axis position
        z: 0,                  // Reset Z-axis position
        ease: "power2.out",    // Easing function
        duration: 0.5,         // Animation duration
      });
    });
